...
function errorMessage(error: unknown) {
  return verificationRequestErrorMessage(error);
}
...
export function VerificationFlowScreen({ target }: { target: VerificationTarget }) {
  ...
  const missingTargetMessage =
    target.kind === "checkout"
      ? "This checkout verification link is incomplete."
      : "This verification link is missing a service-point ID.";
  ...
  useEffect(() => {
    ...
    if (!session.walletId || session.lockStatus !== "unlocked") {
      if (target.kind === "checkout") {
        void setPendingCheckoutVerification({ verificationRequestId: targetId, claimToken: claimToken! });
      } else {
        void setPendingVerificationPublicServicePointId(targetId);
      }
      return;
    }
      ...
      <View style={{ gap: spacing.xl }}>
        ...
        {result ? (
          <Card eyebrow="Authoritative verifier result" heading={result.status}>
            <Text accessibilityLiveRegion="polite" selectable style={[typography.bodyStrong, { color: colors[resultTone] }]}>
